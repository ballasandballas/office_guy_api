...
class AccountingDocumentsAddExpenseRequest(object):
    ...
    swagger_types = {
        'expense_number': 'str',
        'expense_file': 'str',
        'supplier': 'AccountingTypedCustomer',
        '_date': 'datetime',
    ...
    def expense_file(self, expense_file):
        ...
        :type: str
        """
        if expense_file is not None and not re.search(r'^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$', expense_file):  # noqa: E501
            raise ValueError(r"Invalid value for `expense_file`, must be a follow pattern or equal to `/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/`")  # noqa: E501
    ...
    def is_draft(self):
        ...
        Save document as draft<div><i>Leave empty for Draft document</i></div>  # noqa: E501
